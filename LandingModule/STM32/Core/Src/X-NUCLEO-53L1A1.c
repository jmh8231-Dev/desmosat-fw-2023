...
int XNUCLEO53L1A1_I2C3Configure() {
    int status;
    GPIO_InitTypeDef GPIO_InitStruct;
    ...
    XNUCLEO53L1A1_hi2c.Instance = I2C3;
...
static int _ExpanderRd(int I2cExpAddr, int index, uint8_t *data, int n_data) {
    ...
    do {
        status = HAL_I2C_Master_Transmit(&hi2c3, I2cExpAddr, &RegAddr, 1, 100);
        if (status)
            break;
        status = HAL_I2C_Master_Receive(&hi2c3, I2cExpAddr, data, n_data, n_data * 100);
    } while (0);
